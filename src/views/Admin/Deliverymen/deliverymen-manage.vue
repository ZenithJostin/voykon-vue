<template>
  <div class="deliveries-container">
    <div class="title-container">
      <h1>{{ sectionTitle }}</h1>
      <RouterLink class="delivery-export-btn" to="/administrador/deliverymen">
      ...
      <div class="form-container">
        <form @submit.prevent="submitForm" class="row" autocomplete="off">
          <div class="col-md-4 form-group mb-3">
            ...
            <input type="text" id="name" v-model="data.names" class="form-control" required/>
          </div>
          <div class="col-md-4 form-group mb-3">
            <label class="form-label" for="name">Apellidos <span>*</span></label>
            <input type="text" id="name" v-model="data.lastnames" class="form-control" required/>
          </div>
          <div class="col-md-4 form-group mb-3">
...
const sectionTitle = route.params.id ? 'Editar domiciliario' : 'Crear domiciliario'
...
const fetchData = async () => {
  try {
    const response = await axios.get(`${apiBaseUrl}/api/admin/deliveryman/${route.params.id}`)
    data.value = response.data.deliveryman
  } catch (error) {
    Swal.fire({
      icon: 'error',
      title: '¡Error!',
...
const submitForm = async () => {
  const ApiRoute = route.params.id
    ? `${apiBaseUrl}/api/admin/deliveryman/update/${route.params.id}`
    : `${apiBaseUrl}/api/admin/deliveryman/register`
  try {
    ...
    Swal.fire({
      ...
      router.push('/administrador/deliverymen')
    })
  } catch (error) {
